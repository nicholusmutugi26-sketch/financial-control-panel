...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      console.warn('Failed to create audit log for supplementary reject', e)
    }
    ...
    try {
      await sendNotification({
        ...
        title: 'Supplementary Budget Rejected',
        message: `Your supplementary budget request for KES ${updated.amount} has been rejected.`,
        type: 'supplementary_rejected',
        data: {
          supplementaryId: id,
      ...
      console.warn('Failed to send notification for supplementary rejection', e)
    }

    return NextResponse.json({ success: true, supplementary: updated })
  } catch (err) {
    console.error('POST /api/supplementary/[id]/reject error', err)
